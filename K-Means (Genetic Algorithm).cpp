...
void fill_rand(vector<vector<double>> &, int);
double find_mean(vector<double> &);
void initPopul(vector<vector<double>> &, int, int, int, vector<vector<double>> &);
void find_fitness(vector<vector<double>> , int , int , int , vector<vector<double>> &, int, vector<double>, int);
void selection(vector<vector<double>> , int , vector<vector<double>> &, vector<vector<double>>);
void crossover(vector<vector<double>> &, int);
void mutation(vector<vector<double>> , vector<vector<double>> &, int, int, int);
...
int main(){
    ...
    cout << "Enter the number of groups : "; cin >> k;
    ...
    while (1){
        ...
        if (cnt > 2000 || minFit >= minFitness){
            for (int i = 0; i < n; i++){
                ...
                cout << "Student " << i + 1 << " is in Group:" << col + 1 << " with Group Representative:" << popul[prevRow][col] << " | Avg Marks:" << arr[i][m] << " | Marks:{";
